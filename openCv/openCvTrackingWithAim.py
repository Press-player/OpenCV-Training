...
cv2.createTrackbar('thresholdDet','Trackbars',0,200,nothing)
...
#------------------------Show Frames from PiCam------------------------------------------------------------
   ...
   # cv2.moveWindow('FGmask',900,0)
    ...
    for cnt in contours:
        ...
        if area>=thresholdDet:
            cv2.rectangle(merge,(x,y),(x+w,y+h),(255,0,0),3)
#--------------Line--------------------------
            cv2.line(merge,(int(x+(w/2)),0),(int(x+(w/2)),500),(0,255,0),4)  
            #cv2.drawContours(frame,[cnt],0,(0,255,0),3)
            cv2.line(merge,(0,int(y+(h/2))),(500,int(y+(h/2))),(0,255,0),4) 
